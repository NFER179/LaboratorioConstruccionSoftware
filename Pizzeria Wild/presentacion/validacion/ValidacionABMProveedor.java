...
public class ValidacionABMProveedor {
	...
	public boolean GuardarValido(boolean Modificar) {
		...
		String proveedor = this.vtABMProveedor.getTxtProveedorid().getText()
				.trim();
		if (this.mdlProveedor.ExisteProvedor(proveedor) & !Modificar) {
			...
			if (this.vtABMProveedor.getTxtProveedorid().getText().trim()
					.equals("")) {
				mensaje = mensaje + "- Debe Cargar un Id de Proveedor.\n";
				this.vtABMProveedor.getTxtProveedorid()
						.setBackground(Color.RED);
				valido = false;
			}

			if (this.vtABMProveedor.getTxtNombreproveedor().getText().trim()
					.equals("")) {
				mensaje = mensaje + "- Debe Ingresar un Nombre de Proveedor.\n";
				this.vtABMProveedor.getTxtNombreproveedor().setBackground(
						Color.RED);
				valido = false;
			}

			if (this.vtABMProveedor.getTxtTelefono().getText().trim()
					.equals("")) {
				mensaje = mensaje
						+ "- Debe Ingresar un Telefono Para Poder Contactar al Proveedor.\n";
			...
			if (this.vtABMProveedor.getTxtMail().getText().trim().equals("")) {
				mensaje = mensaje
						+ "- Debe Ingresar una Direccion de Mail Para poder Enviar las Solicitudes.\n";
