...
public class ControladorSeleccionMateriaPrima implements ActionListener {
	...
	public ControladorSeleccionMateriaPrima(
			ControladorCategoriaDetalle CtrCategoriaDetalle,
			CategoriaDetalleVista CategoriaDetalle) {
	...
	private void CargarTabla() {
		...
		this.vtSeleccionMT.getModelTable().setColumnIdentifiers(
				this.vtSeleccionMT.getNombreColumnas());

		for (MateriaPrimaDTO mt : this.mdlMateriaPrima
				.ObtenerMateriasPrimasSinAsignar()) {
			if (this.NoAsignada(mt.getNombre(),
					this.ctrCategoriaDetalle.MateriasPrimasEnTabla())) {
	...
	private boolean NoAsignada(String MateriaPrima,
			List<MateriaPrimaDTO> Asignadas) {
	...
	// 0).toString().trim();
	// this.mdlMateriaPrima.AsignarACategoria(MateriaPrima,
	// this.ctrCategoriaDetalle.GetIdCategoria());
	// }
	//
	// this.CargarTabla();
