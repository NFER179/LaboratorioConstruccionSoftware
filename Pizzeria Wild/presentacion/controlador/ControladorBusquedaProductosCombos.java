...
public class ControladorBusquedaProductosCombos implements ActionListener {
	...
	public ControladorBusquedaProductosCombos(ControladorABMCombo Ctr, ABMComboVista Vista) {
		this.ctr = Ctr;
	...
	public void Inicializar() {
		this.CargarComboBox();
		String product = this.vtBusqueda.getComboBox().getSelectedItem().toString().trim();
	...
	private void CargarTabla(String producto) {
		...
		this.vtBusqueda.getModelTable().setColumnIdentifiers(this.vtBusqueda.getNombreColumna());
	...
	private void RecargarTabla() {
		if(this.vtBusqueda.getModelCbx().getSize() > 0) {
			String p = this.vtBusqueda.getComboBox().getSelectedItem().toString().trim();
	...
	private void AceptarProducto() {
		String producto = this.vtBusqueda.getComboBox().getSelectedItem().toString().trim();
		
		JTable t = this.vtBusqueda.getTable();
		int selectedRow = t.getSelectedRow(); 
		String sabor = t.getValueAt(selectedRow, 0).toString().trim();
		
		int cantidad = Integer.parseInt(this.vtBusqueda.getTxtCantidad().getText().trim());
