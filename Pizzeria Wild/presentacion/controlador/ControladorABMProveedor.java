...
public class ControladorABMProveedor implements ActionListener {
	...
	public void InicializarModificacion(String ProveedorId) {
		this.Modificar = true;
		ProveedorDTO proveedor = this.mdlProveedor.ObtenerProveedor(ProveedorId);
	...
	private void CargarDatosProveedor(ProveedorDTO proveedor) {
		this.vtABMProveedor.getTxtProveedorid().setText(proveedor.getProveedorId());
		...
		this.vtABMProveedor.getTxtNombreproveedor().setText(proveedor.getNombre());
	...
	private void CargarCategoriasProveedor(ProveedorDTO proveedor) {
		...
		this.vtABMProveedor.getModelCategoria().setColumnIdentifiers(this.vtABMProveedor.getNombreColumnasCategoria());
		for(CategoriaDTO c:this.mdlProveedor.ObtenerCategorias(proveedor.getProveedorId())) {
			Object[] fila = {c.getIdCategoria(), c.getDescripcion()};
			this.vtABMProveedor.getModelCategoria().addRow(fila);
		}
		this.vtABMProveedor.getTblCategoria().setModel(this.vtABMProveedor.getModelCategoria());
	}
	...
	private void CargarmMTProveedor(ProveedorDTO proveedor) {
		...
		this.vtABMProveedor.getModelMT().setColumnIdentifiers(this.vtABMProveedor.getNombreColumnasMT());
		for(MateriaPrimaDTO mp:this.mdlProveedor.ObtenerMateriasPrimasDeProveedor(proveedor.getProveedorId())) {
			Object[] fila = {mp.getNombre()};
			this.vtABMProveedor.getModelMT().addRow(fila);
		}
		this.vtABMProveedor.getTblMateriaPrima().setModel(this.vtABMProveedor.getModelMT());
	}
	...
	private void AgregarCategoria() {
		ControladorBusquedaCategoriasProveedor ctr = new ControladorBusquedaCategoriasProveedor(this, this.vtABMProveedor);
		ctr.Inicializar();
	}
	...
	private void QuitarCategoria() {
		...
		for(int i = rowSelection.length - 1 ; i >= 0  ; i--) {
			String categoriID = t.getValueAt(rowSelection[i], 0).toString().trim();
			String desripcion = t.getValueAt(rowSelection[i], 1).toString().trim();
			...
			for(int j = this.vtABMProveedor.getModelMT().getRowCount() - 1 ; j >= 0 ; j--) {
				String mp = this.vtABMProveedor.getModelMT().getValueAt(j, 0).toString().trim();
				if(MateriaPrimaModelo.SeEncuentraEn(mp, this.mdlCategoria.ObtenerMateriasPrimasPara(c.getIdCategoria()))){
	...
	private void AgregarMateriaPrima() {
		ControladorBusquedaMPProveedor ctr = new ControladorBusquedaMPProveedor(this, this.vtABMProveedor);
		ctr.Inicializar();
	}
	...
	private void Guardar(boolean modificar) {
		String ProveedorId = this.vtABMProveedor.getTxtProveedorid().getText().trim();
		String Nombre = this.vtABMProveedor.getTxtNombreproveedor().getText().trim();
		...
		ProveedorDTO pr = new ProveedorDTO(ProveedorId, Nombre, Telefono, Mail, Activo);
